...
import within30.com.wheel.transformer.FadingSelectionTransformer;
import within30.com.wheel.transformer.ScalingItemTransformer;
import within30.com.wheel.transformer.WheelItemTransformer;
import within30.com.wheel.transformer.WheelSelectionTransformer;
...
public class WheelView extends View {
    ...
    Context context;
    Activity activity;
    public WheelView(Context context,Activity activity) {
        super(context);
    ...
    public WheelView(Context context, AttributeSet attrs, int defStyle) {
       ...
       // d = getResources().getDrawable(R.drawable.law);
    ...
    private void drawWheelItems(final Canvas canvas) {
       /* // drawables items array
        ...
        // drawables items array
        ArrayList<Integer> noServicecategoryDrawablesList = new ArrayList<Integer>();
        noServicecategoryDrawablesList.add(R.mipmap.noservice_salon);
        noServicecategoryDrawablesList.add(R.mipmap.noservice_spa_and_massage);
        noServicecategoryDrawablesList.add(R.mipmap.dentist);
        noServicecategoryDrawablesList.add(R.mipmap.diagntcs);
        noServicecategoryDrawablesList.add(R.mipmap.noservice_autoservices);*/
        ...
        for (int i = offset; i < length; i++) {
            ...
            if (Rect.intersects(sTempRect, mViewBounds)) {
                ...
                if (servicesDOList.get(adapterPosition).isActive()) {
                    id = w30Utilities.getServiceImage(W30Utilities.SERVICE_LANDING_ACTIVE,servicesDOList.get(adapterPosition).getName());
                }else {
                    id = w30Utilities.getServiceImage(W30Utilities.SERVICE_LANDING_INACTIVE,servicesDOList.get(adapterPosition).getName());
                }
                ...
                if (drawable != null) {
                    ...
                    Typeface tf = Typeface.createFromAsset(getResources().getAssets(), "fonts/DroidSerif.ttf"); ;
                    ...
                    if (servicesDOList.get(adapterPosition).isActive()) {
                        ...
                        int fontsize_mormal = getResources().getDimensionPixelSize(R.dimen.text_size_normal);
                        imagePaint.setTextSize(fontsize_mormal);
                    }
                  ...
                  /*  if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M){
                        ...
                        imagePaint.setTextSize(40f);
                    }*/
                    //do call
                    if (servicesDOList.get(adapterPosition).isActive()) {
                       imagePaint.setColor(getResources().getColor(R.color.w30_blue));
                        tf = Typeface.createFromAsset(getResources().getAssets(), "fonts/DroidSerif_Bold.ttf");
                    }else {
                        imagePaint.setColor(getResources().getColor(R.color.noservice_grey));
                        tf = Typeface.createFromAsset(getResources().getAssets(), "fonts/DroidSerif.ttf");
                    }
